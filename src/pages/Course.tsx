...
const CoursePage: React.FC = () => {
  ...
  if (!course) return <Alert type="info" message="Курс не найден" style={{ margin: 40 }} />;
    ...
    <Card
      title={<div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {user && (user.role === 'admin' || user.role === 'teacher') && (
          <>
            <Button type="default" size="small" style={{ marginLeft: 16 }} onClick={() => { setEditDialogOpen(true); setEditValues(course); }}>Редактировать</Button>
            <Button type="default" danger size="small" style={{ marginLeft: 8 }} onClick={async () => {
              if (window.confirm('Вы действительно хотите удалить этот курс?')) {
                ...
                try {
                  ...
                  message.error(e.message || 'Ошибка удаления курса');
                } finally {
                  setDeleteLoading(false);
                }
              }
            }} disabled={deleteLoading}>Удалить</Button>
          </>
        )}
      </div>}
        ...
        {editValues && (
          <form onSubmit={async e => {
            ...
            try {
              ...
              message.error(err.message || 'Ошибка обновления курса');
            }
          }}>
            <div style={{ marginBottom: 12 }}>
              <input type="text" value={editValues.name} onChange={e => setEditValues({ ...editValues, name: e.target.value })} placeholder="Название курса" style={{ width: '100%', padding: 8, fontSize: 16 }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <input type="text" value={editValues.language} onChange={e => setEditValues({ ...editValues, language: e.target.value })} placeholder="Язык" style={{ width: '100%', padding: 8 }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <textarea value={editValues.content} onChange={e => setEditValues({ ...editValues, content: e.target.value })} placeholder="Описание" style={{ width: '100%', padding: 8, minHeight: 80 }} />
            </div>
